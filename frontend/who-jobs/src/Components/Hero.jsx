...
export default function Hero({ role, setRole, data }) {
  ...
  async function handleSubmit(e) {
    ...
    setStatus("loading");
    try {
      await subscribeEmail(email, role);
      setStatus("done");
    } catch {
      setStatus("error");
    }
  ...
  return (
    <section ref={heroRef} className="relative overflow-hidden px-6 pb-28 pt-16 lg:pt-24">
      {floatingTags.map((tag) => (
        <div key={tag.label} {...tag} scrollYProgress={scrollYProgress} reduce={reduce} />
      ))}
      ...
      <div className="relative z-10 mx-auto grid max-w-6xl grid-cols-1 items-center gap-16 lg:grid-cols-[1.15fr_0.85fr]">
        <div>
          ...
          <form onSubmit={handleSubmit} className="mt-9 flex max-w-md flex-col gap-3 sm:flex-row">
            <div
