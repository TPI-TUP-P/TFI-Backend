...
const APPLY_ROLES = [0, 2, 3];
const ADMIN_ROLES = [2, 3];
  ...
  return (
    <>
      <article
        ...
        className="w-full cursor-pointer rounded-xl border border-brand-border bg-brand-card px-3 py-3 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md sm:px-4"
      >
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4">

          <div className="min-w-0 flex-1">
            <div className="flex min-w-0 flex-wrap items-center gap-2">
              <h2 className="min-w-0 truncate text-base font-bold text-brand-title sm:text-lg">
                {job.job_position}
              </h2>
          ...
          >
            {canApply && (
              alreadyApplied ? (
                postulationState === 1 ? (
                  <span className="rounded-lg bg-green-100 px-3 py-2 text-center text-sm font-semibold text-green-700">
                    ✓ Aceptada
                  </span>
                ) : postulationState === 2 ? (
                  <span className="rounded-lg bg-red-100 px-3 py-2 text-center text-sm font-semibold text-red-700">
                    ✕ Rechazada
                  </span>
                ) : (
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                    <span className="rounded-lg bg-brand-bg px-3 py-2 text-center text-sm font-semibold text-brand-muted">
                      ✓ Postulado
                    </span>
                    <button
            ...
            {canManage && (
              <button
                ...
                className="flex h-9 w-9 shrink-0 items-center justify-center self-end rounded-lg border border-brand-border text-brand-muted transition hover:border-red-300 hover:bg-red-50 hover:text-red-600 sm:self-auto"
              >
                ✕
              </button>
            )}
